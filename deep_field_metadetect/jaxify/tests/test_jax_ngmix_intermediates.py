...
class TestJaxNgmixIntermediates:
    """Test that JAX and ngmix versions produce the same intermediate values."""
    ...
    def test_gauss_reconv_psf_consistency(self, simple_obs_pair):
        """Test that JAX and ngmix produce the same Gaussian reconvolution PSF."""
    ...
    def test_max_gauss_reconv_psf_consistency(self, simple_obs_pair):
        """Test that JAX and ngmix produce the similar Gaussian reconvolution PSF.
        ...
        # Test PSF properties
        assert np.allclose(
            max_reconv_psf_ngmix.fwhm, 
            max_reconv_psf_jax.fwhm, 
            rtol=0.02, atol=1e-6
    ...
    def test_metacal_single_shear_consistency(self, simple_obs_pair):
        """Test that JAX and ngmix produce consistent results for single shear operations."""
    ...
    def test_metacal_shears_intermediate_values(self, simple_obs_pair):
        ...
        for shear in test_shears:
            ...
            assert img_mean_diff < 1e-4, f"Shear {shear}: Image mean difference too large: {img_mean_diff}"
            ...
            assert 0.99 < weight_ratio < 1.01, f"Shear {shear}: Weight ratio out of range: {weight_ratio}"
